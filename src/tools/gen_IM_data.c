...
const char USAGE[] =
	"Usage: %s [-w|--show-warning] <cin_filename>\n"
;
 ...
 * "from" and "to" fields.
 */
void find_keyin_sequence(const IntervalType range[], KeySeqWord keyin_buf[], int width)
{
...
const char *enumerate_keyin_sequence(const char *phrase, long phr_pos, int32_t total_freq)
{
	...
	KeySeqWord keyin_buf[MAX_PHRASE_LEN + 1] = {0};
	const char *p = phrase;
	PhraseData tmpPhrase;
	WordData tmpWord;
	WordData *q;
	WordData *r;
	int b;
	...
	tmpWord.text = &tmpPhrase;
	for (i = 0; *p; i++) {
		b = ueBytesFromChar(*p);
		strncpy(tmpWord.text->phrase, p, b);
	...
	if (range[1].from == range[1].to) {
		...
		int old_num_phrase_data = num_phrase_data;
...
int main(int argc, char *argv[])
{
	...
	printf("Opening total-frequency table (%s)... ", FREQ_FILE);
	...
	if (!freq) {
		putchar('\n');
		fprintf(stderr, "%s: Error reading total-frequency table.\n", argv[0]);
		exit(-1);
	}
	puts("done.");
